...
class Ancs extends CI_Controller
{
    //------------------------------------------------------------------------------------------------------------------
    /*
     * Global parameter
    ...
    public function __construct()
    {
        ...
        $this->load->model('Ancs_model', 'ancs');
    ...
    public function index()
    {
        ...
        $this->layout->view('ancs/index_view', $data);
    }
    ...
    public function get_list()
    {
        ...
        $this->ancs->owner_id = $this->owner_id;
        $rs = $this->ancs->get_list($start, $limit);
    ...
    public function get_list_by_house()
    {
        ...
        $this->ancs->owner_id = $this->owner_id;
        $rs = $this->ancs->get_list_by_house($house_id);
    ...
    {
        $this->ancs->owner_id = $this->owner_id;
        $total = $this->ancs->get_list_total();
    ...
    {
        $this->ancs->owner_id = $this->owner_id;
        $total = $this->ancs->get_list_by_village_total();
        $json = '{"success": true, "total": '.$total.'}';
    ...
    */
     ...
     */
    public function get_anc_vaccine_list()
    {
        $rs = $this->basic->get_anc_vaccine_list();
    ...
    public function save_service()
    {
        ...
        else
        {
            ...
            {
                $this->ancs->owner_id = $this->owner_id;
                $this->ancs->user_id = $this->user_id;
                $this->ancs->provider_id = $this->provider_id;

                $rs = $this->ancs->save_service($data);
    ...
    public function get_anc_visit_list()
    {
    ...
    public function get_anc_visit_history()
    {
        $hn = $this->input->post('hn');

        if(!empty($hn))
        {
            $rs = $this->ancs->get_history($hn);
    ...
    public function remove_anc_register() {
        $person_id = $this->input->post('person_id');
        
        if(empty($person_id)) {
            $json = '{ "success": false, "msg": "No person id found." }';
        } else {
            $rs = $this->ancs->remove_ncd_register($person_id);
            if($rs) {
                $json = '{ "success": true }';
            } else {
                $json = '{ "success": false, "msg": "Cann\'t remove anc register." }';
            }
        }
